from app.core.repositories import UserRolesPermissionsInterface
from fastapi import Request,status,HTTPException, Depends
from app.api.dependencies import get_user_roles_permissions
from typing import Callable, Any
from functools import wraps

#route protection validators
def requires_traveller(func: Callable)->Callable:
    """
    Decorator that uses FastAPI's dependency injection internally
    """
    @wraps(func)
    async def wrapper(
        request: Request, 
        user_roles_permissions_repo:UserRolesPermissionsInterface= Depends(get_user_roles_permissions),
        *args: Any, 
        **kwargs: Any
    )->Any:
        user_id = getattr(request.state, 'user_id', None)

        if not user_id:
            raise HTTPException(
                status_code = status.HTTP_401_UNAUTHORIZED,
                detail='Authentication required'
            )
        
        #Get user roles and permissions using the repo injected
        user_roles_permissions = await user_roles_permissions_repo.get_user_roles_and_permissions(user_id)
        if not user_roles_permissions.is_active:
            raise HTTPException(
                status_code = status.HTTP_403_FORBIDDEN,
                detial='Account has been deactivated'
            )

        return await func(request, *args, **kwargs)
    return wrapper